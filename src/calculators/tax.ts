...
export function calculateTax(inputs: TaxInputs): TaxResult {
  ...
  const esgCapByIncome = totalIncome * ESG_INCOME_PCT_CAP
  const thaiEsgXDeduction = Math.min(Math.max(thaiEsgX, 0), ESG_CAP, esgCapByIncome)
  const thaiEsgXFromLtfDeduction = Math.min(Math.max(thaiEsgXFromLtf, 0), ESG_CAP, esgCapByIncome)
